    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {projects.map((project) => (
          <div key={project.id} className="glass-panel p-6 flex flex-col group hover:border-blue-500/50 transition-all">
            <div className="flex justify-between items-start mb-4">
              <h3 className="text-lg font-bold text-gray-100 group-hover:text-blue-400 transition-colors">{project.name}</h3>
              <span className={`px-2 py-1 rounded text-xs font-medium border ${
                project.status === 'Active' ? 'bg-blue-500/10 text-blue-400 border-blue-500/30' :
            ...
            <div className="space-y-4 flex-1">
              <div>
                ...
                <div className="w-full bg-gray-800 rounded-full h-2">
                  <div className="bg-blue-500 h-2 rounded-full shadow-[0_0_8px_rgba(52,211,153,0.6)]" style={{ width: `${project.progress}%` }}></div>
                </div>
              </div>
            ...
            <div className="mt-6 pt-4 border-t border-gray-800 flex justify-between items-center">
              <div className="flex -space-x-2">
                {[1, 2, 3].map((i) => (
                  <div key={i} className={`w-8 h-8 rounded-full border-2 border-gray-900 bg-gray-700 flex items-center justify-center text-xs font-bold ${i === 1 ? 'text-blue-400' : 'text-gray-300'}`}>
                    U{i}
                  </div>
                ))}
              </div>
          ...
          {[
            ...
            <div key={i} className="py-4 flex items-start gap-4 hover:bg-gray-800/20 px-2 rounded-lg transition-colors group cursor-pointer">
              ...
              <div className="flex-1">
                <p className="text-gray-200 font-medium group-hover:text-blue-400 transition-colors">{task.title}</p>
