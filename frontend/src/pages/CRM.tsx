...
export default function CRM() {
  ...
  const pipelineStages = [
    ...
    { name: 'Won', color: 'border-emerald-500' }
  ];
    ...
    <div className="space-y-6 h-full flex flex-col">
      ...
      <div className="flex items-center border-b border-gray-800 gap-6">
        {['pipeline', 'customers', 'leads'].map((tab) => (
          ...
          >
            {tab}
            {activeTab === tab && (
              <div className="absolute bottom-0 left-0 w-full h-0.5 bg-emerald-400 rounded-t-full shadow-[0_0_8px_rgba(52,211,153,0.8)]"></div>
            )}
          </button>
        ))}
      </div>
              ...
              <div key={stage.name} className="w-80 flex flex-col glass-panel border-t-4 bg-gray-900/30" style={{ borderTopColor: 'var(--tw-border-opacity, 1)' }}>
                ...
                <div className="p-4 flex-1 space-y-4 overflow-y-auto min-h-[300px]">
                  {deals.filter(d => d.stage === stage.name).map(deal => (
                    <div key={deal.id} className="glass-panel p-4 cursor-grab hover:border-emerald-500/50 transition-colors group">
                      <div className="flex justify-between items-start mb-2">
                        <h4 className="text-gray-100 font-medium text-sm leading-tight group-hover:text-emerald-400 transition-colors">{deal.title}</h4>
                ...
                <tr key={i} className="hover:bg-gray-800/30 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      <button className="text-gray-400 hover:text-emerald-400 transition-colors"><Mail className="h-4 w-4" /></button>
                      <button className="text-gray-400 hover:text-emerald-400 transition-colors"><Phone className="h-4 w-4" /></button>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span className="px-2 py-1 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-full text-xs font-medium">Active</span>
                  </td>
      ...
      {activeTab === 'leads' && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <div className="w-16 h-16 rounded-full bg-emerald-500/10 flex items-center justify-center border border-emerald-500/30 mb-4">
          ...
          <button className="bg-emerald-500 text-black px-6 py-2 rounded-lg font-semibold hover:bg-emerald-400 transition-colors">
            Import Leads
          </button>
        </div>
      )}
