...
export default function Chatbot() {
  ...
  const [messages, setMessages] = useState<{role: 'user' | 'bot', text: string}[]>([
    { role: 'bot', text: 'Hello! I am the Avenik Core AI Assistant. How can I help you find the right financial scheme today?' }
  ]);
    ...
    <>
      ...
      {!isOpen && (
        <button 
          ...
          className="fixed bottom-6 right-6 p-4 bg-emerald-600 text-white rounded-full shadow-xl hover:bg-emerald-700 hover:-translate-y-1 transition-all z-50 flex items-center justify-center"
        >
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed bottom-6 right-6 w-80 sm:w-96 bg-gray-900/60 backdrop-blur-md border-gray-800 rounded-2xl shadow-2xl border border-gray-700 z-50 overflow-hidden flex flex-col h-[500px] max-h-[80vh]">
          {/* Header */}
          <div className="bg-emerald-600/90 backdrop-blur-md text-white p-4 flex justify-between items-center shadow-lg border-b border-emerald-500/50">
            ...
            <button onClick={() => setIsOpen(false)} className="text-white hover:text-emerald-200 bg-white/10 p-1 rounded-md transition-colors">
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-800/50">
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] p-3 rounded-2xl ${msg.role === 'user' ? 'bg-emerald-600 text-white rounded-br-none' : 'bg-gray-900/60 backdrop-blur-md border-gray-800 border text-gray-100 rounded-bl-none shadow-sm'}`}>
          ...
          {/* Input Area */}
          <div className="p-4 bg-gray-900/60 backdrop-blur-md border-gray-800 border-t">
            <div className="flex gap-2">
              ...
              <button 
                onClick={handleSend}
                className="p-2 bg-emerald-600 text-white rounded-full hover:bg-emerald-700 flex-shrink-0"
              >
